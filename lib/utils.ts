...
// Complete map of ISO 3166-1 alpha-2 Country Codes to ISO 4217 Currency Codes
const COUNTRY_TO_CURRENCY: Record<string, string> = {
  ...
  // Currency codes passed directly (fallback)
  USD: 'USD', GBP: 'GBP', EUR: 'EUR', AED: 'AED', INR: 'INR',
  JPY: 'JPY', CAD: 'CAD', AUD: 'AUD', CHF: 'CHF',
};

export function getCurrencySymbol(currencyCode?: string): string {
  const code = (currencyCode || 'IN').toUpperCase();

  // Resolve to ISO currency code
  const isoCurrency = COUNTRY_TO_CURRENCY[code] || null;

  if (isoCurrency) {
    try {
      // Use Intl API to get the correct symbol dynamically
      ...
      // Extract symbol: "€0" → "€", "$0" → "$", "AED 0" → "AED "
      return formatted.replace(/[\d,.\s]+$/, '').trim() + (isoCurrency === 'AED' ? ' ' : '');
    } catch (e) {
      // Intl failed — use hardcoded fallback
    }
  }

  // Hardcoded fallbacks for known currencies
  if (['USD', 'CAD', 'AUD', 'NZD', 'SGD', 'HKD'].includes(isoCurrency || '')) return '$';
  if (isoCurrency === 'GBP') return '£';
  if (isoCurrency === 'EUR') return '€';
  if (isoCurrency === 'AED') return 'AED ';
  if (isoCurrency === 'JPY') return '¥';
